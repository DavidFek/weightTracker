...
window.onload = () => {
    ...
    weightEntries = storedEntries.map((entry) => (Object.assign(Object.assign({}, entry), { date: new Date(entry.date) })));
...
function displayWeightEntries() {
    let tableBody = document.querySelector(".weight-list tbody");
    tableBody.innerHTML = '';
    let storedEntries = JSON.parse(localStorage.getItem('weightEntries') || '[]');
    weightEntries = storedEntries.map((entry) => (Object.assign(Object.assign({}, entry), { date: new Date(entry.date) })));
...
function updateCurrentWeight() {
    ...
    let currentWeightElement = document.querySelector(".weight-statistics__stats-current-weight");
...
function createChart(period) {
    ...
    let startWeightElement = document.querySelector(".weight-statistics__stats-start-weight");
    startWeightElement.textContent = data[0].toString() + " kg";
    let progressWeightElement = document.querySelector(".weight-statistics__stats-progress-weight");
